...
export const generateTimetable = async (): Promise<TimetableResult> => {
    ...
    for (const leave of approvedLeave) {
        ...
        // Find all classes this faculty teaches
        const facultyCourses = allCourses.filter(c => faculty.expertise.includes(c.code));

        for (const course of facultyCourses) {
            const studentGroupsForCourse = allStudentGroups.filter(sg => sg.courses.includes(course.code));
            for (const group of studentGroupsForCourse) {
                 // A simple way to represent a full day leave - block all slots.
    ...
    for (const { course, studentGroup } of allCoursesToSchedule) {
      ...
      for (const day of DAYS) {
        ...
        // This is a simplification. A real implementation would check the actual date against the leave interval.
        const facultyIsOnLeaveToday = approvedLeave.some(leave => 
            leave.facultyId === suitableFaculty.id && 
            // A basic check, would need a proper date mapping in a real system
            ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].includes(day)
        );

        if (facultyIsOnLeaveToday) continue; // Skip scheduling for this faculty on this day
